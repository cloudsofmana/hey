import type { FC } from 'react';

import Loader from '@components/Shared/Loader';
import { getAuthApiHeaders } from '@helpers/getAuthApiHeaders';
import { HEY_API_URL } from '@hey/data/constants';
import { CardHeader, ErrorMessage, NumberedStat } from '@hey/ui';
import { useQuery } from '@tanstack/react-query';
import axios from 'axios';

export interface StatsType {
  allTime: string;
  lastHour: string;
  thisMonth: string;
  thisWeek: string;
  today: string;
  yesterday: string;
}

const Numbers: FC = () => {
  const getEventsNumberStats = async (): Promise<StatsType> => {
    const response: {
      data: StatsType;
    } = await axios.get(`${HEY_API_URL}/internal/leafwatch/stats/events`, {
      headers: getAuthApiHeaders()
    });

    return response.data;
  };

  const { data, error, isLoading } = useQuery({
    queryFn: getEventsNumberStats,
    queryKey: ['getEventsNumberStats'],
    refetchInterval: 2500
  });

  if (isLoading) {
    return <Loader className="my-10" message="Loading events stats..." />;
  }

  if (error) {
    return (
      <ErrorMessage
        className="m-5"
        error={error}
        title="Failed to load events stats"
      />
    );
  }

  if (!data) {
    return <div className="m-5">No events stats found.</div>;
  }

  return (
    <div>
      <CardHeader title="Events" />
      <div className="m-5 grid grid-cols-2 gap-2 sm:grid-cols-3">
        <NumberedStat count={data.lastHour} name="Last 1 hour" />
        <NumberedStat count={data.today} name="Today" />
        <NumberedStat count={data.yesterday} name="Yesterday" />
        <NumberedStat count={data.thisWeek} name="This week" />
        <NumberedStat count={data.thisMonth} name="This month" />
        <NumberedStat count={data.allTime} name="All time" />
      </div>
    </div>
  );
};

export default Numbers;
